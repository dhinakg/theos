...
before-stage::
	...
	$(ECHO_NOTHING)mkdir -p "$(THEOS_STAGING_DIR)/$(INSTALL_PREFIX)"$(ECHO_END)
else # _THEOS_TOP_INVOCATION_DONE
before-stage::
	@:
...
internal-stage::
	$(ECHO_NOTHING)[ -d $(THEOS_LAYOUT_DIR_NAME) ] && rsync -a "$(THEOS_LAYOUT_DIR_NAME)/" "$(THEOS_STAGING_DIR)/$(INSTALL_PREFIX)" --exclude "DEBIAN" $(_THEOS_RSYNC_EXCLUDE_COMMANDLINE) || true$(ECHO_END)
